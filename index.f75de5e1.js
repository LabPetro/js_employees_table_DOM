...
function sortByOrder(cellIndex) {
    rows.sort((a, b)=>{
        ...
        if (cellIndex === 3 || cellIndex === 4) {
            if (isAsc) return Number(aVal) - Number(bVal);
            else return Number(bVal) - Number(aVal);
        } else {
            if (isAsc) return aVal.localeCompare(bVal, {
                numeric: "true",
                sensitivity: "base"
            });
...
function addRow(formData) {
    ...
    dataArray.forEach((val, index)=>{
        ...
        if (index === 4) cell.textContent = "$" + new Intl.NumberFormat("en-US").format(val);
